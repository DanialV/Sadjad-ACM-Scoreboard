        ...
        if(req.xhr){
            db.users.find({},{codeforces_username : true , _id : true}).lean().exec(function(err1,users){
                if(err1){
                    ...
                    res.render('codeforces_update',{ data : {status_code : '500',status_massage : 'Internal Server error' , type : 'Error'}});
                }
                else{
                            ...
                            code_req(url, function (err3, response, body) {
                                if (response && !err3 && response.statusCode == 200) {
                                    ...
                                    if (body.status == "OK") {
                                        ...
                                        res.render('codeforces_update',{ data : {'message' : 'Codeforces information gotten form codeforces server successfully', 'type' : 'OK'}});
                                    }
                                }
                                else {
                                    ...
                                    res.render('codeforces_update',{ data : {status_code : '500',status_massage : 'Codeforces Api is Down or We have internet connection problem' , type : 'Error'}});
                                }
                            });
                        }
                    });
